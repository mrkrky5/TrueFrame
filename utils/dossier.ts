import { HistoryCard, MediaType, AccuracyType, SpoilerLevel } from "@/types";
  ...
  cards.forEach(card => {
    ...
    if (!dossiersMap[slug]) {
      dossiersMap[slug] = {
        ...
        flagshipCount: 0,
        totalReadingTime: 0,
        accuracyDistribution: { "real": 0, "partly-real": 0, "fictionalized": 0, "inspired-by-reality": 0, "fiction": 0 },
        spoilerSummary: "none",
        topTags: []
      };
    }
    ...
    // Avoid duplicate cards if the mapping merges titles
    if (!d.cardIds.includes(card.id)) {
      ...
      if (card.accuracyType) {
        d.accuracyDistribution[card.accuracyType]++;
      }

      if (card.spoilerLevel === "major") d.spoilerSummary = "major";
      else if (card.spoilerLevel === "minor" && d.spoilerSummary !== "major") d.spoilerSummary = "minor";

      if (card.tags) {
        d.topTags = Array.from(new Set([...d.topTags, ...card.tags]));
      }
    }
  });

  // Ranking: Flagship count (desc) > Total card count (desc) > Reading time (desc)
...
/**
 * Returns dossiers with at least 3 cards and 1 flagship, or 2 cards and 1 flagship.
 */
export function getStrongDossiers(cards: HistoryCard[]): MediaDossier[] {
  return getAllDossiers(cards).filter(d => 
    (d.cardIds.length >= 2 && d.flagshipCount >= 1) || d.cardIds.length >= 5
  );
}
